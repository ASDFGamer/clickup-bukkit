...
import org.asdfgamer.bukkit.clickup.model.AssigneeTask;
...
public interface TaskService
{
    ...
    Call<TaskList> listTasks(@Path("list_id") String list_id, @Query("archived") boolean archieved);

    @GET("api/v2/list/{list_id}/task")
    Call<TaskList> listTasks(@Path("list_id") String list_id, @Query("archived") boolean archieved, @Query("parent") String parent);
    ...
    @PUT("api/v2/task/{task_id}")
    Call<Task> updateStatus(@Path("task_id") String task_id, @Body AssigneeTask task);
